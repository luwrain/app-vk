...
abstract class FollowingsArea extends ListArea
{
    ...
    private SuggestionsArea suggestionsArea = null;

    FollowingsArea(Luwrain luwrain, Strings strings, Base base, Actions actions, Runnable closing)
    {
	...
	if (event.isSpecial() && !event.isModified())
		...
		if (suggestionsArea == null)
		    return false;
		luwrain.setActiveArea(suggestionsArea);
		return true;
	    case BACKSPACE:
		if (defaultArea == null)
		    return false;
    ...
    {
	return actions.lists.getFollowingsActions();
    }
    ...
    void setSuggestionsArea(SuggestionsArea suggestionsArea)
    {
	NullCheck.notNull(suggestionsArea, "suggestionsArea");
	if (this.suggestionsArea != null)
	    throw new RuntimeException("suggestionsArea already set");
	...
	params.name = strings.followingsAreaName();
	return params;
    }
